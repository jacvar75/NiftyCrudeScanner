...
WEIGHT_FILE = "nifty_feature_weights.json"
REQUIRED_TRADES_FOR_UPDATE = 100
WEIGHT_LEARNING_RATE = 0.2
MIN_CORRELATION = 0.15
FEATURE_NAMES = ["rvol", "oi_acceleration", "value_area", "strike_rotation", "call_wall", "put_wall", "breakout_acceptance"]
...
def load_weights():
    ...
    try:
        ...
        uniform = 10.0 / len(FEATURE_NAMES)
        feature_weights = {f: uniform for f in FEATURE_NAMES}
        save_weights()
        logging.info(f"Seeded initial uniform weights: {feature_weights}")
...
def update_weights_from_trades():
    with update_lock:
        ...
        summary_file = os.path.join(LOG_DIR, "nifty_orderflow_sim_summary.csv")
        ...
        try:
                ...
                if opt_symbol:
                    try:
                        ...
                        if opt_quote is not None:
                            ...
                            # Quote fetch failed – check stale duration
                        ...
                        logging.warning(
                            f"⚠️ NIFTY option premium refresh failed for {opt_symbol} (using stale price {active_trade.get('option_ltp')}): {e}")
            ...
            if active_trade is None:
                ...
                active_trade = {
                    ...
                    "last_quote_time": now,
                    "feature_snapshot": {k: v['value'] if not isinstance(v, dict) else v for k, v in feature_scores.items()}
                }
                save_state()
...
@app.route('/')
def index():
    return send_from_directory('templates', 'orderflow_nifty.html')
